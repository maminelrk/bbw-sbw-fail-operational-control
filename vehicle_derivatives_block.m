function dx = vehicle_derivatives_block(x, delta_cmd, Fx_cmd, mask, p, knownMask)
% x = [vx vy r X Y psi delta FxFL FxFR FxRL FxRR omegaA omegaB] (13x1)
% An 11-state legacy call retains the old equivalent steering servo.
% delta_cmd : road-wheel / angle de roue command [rad]
...
if nargin < 5 || isempty(p), p = get_params(); end
if nargin < 4, mask = ones(6,1); end
...
q = vehicle_dynamics_quantities(x, p, channels);

%% Dynamique d'actionneur (saturation + limite de vitesse)
delta_cmd_sat = min(max(delta_cmd, -p.delta_max), p.delta_max);
if numel(x) == 13
    [ddelta,dmotor] = steering_actuator_dynamics(delta_act,x(12:13),delta_cmd_sat,channels,p,knownMask);
else
    availableRate = min(p.delta_rate,sum(p.steering_channel_rate.*channels(5:6)));
    ddelta = sign(delta_cmd_sat-delta_act)*min(abs(delta_cmd_sat-delta_act)/p.actuator_tau_delta,availableRate);
    dmotor = [];
end

% Brake-only actuator: command range is [-mu*Fz_dynamic, 0].
Fx_cmd_sat = min(max(Fx_cmd(:), -p.mu*q.Fz), 0);
Fx_cmd_sat = Fx_cmd_sat.*channels(1:4);
...
dx = [vx_dot; vy_dot; r_dot; X_dot; Y_dot; psi_dot; ddelta; dFx(:); dmotor];
