...
The tested source is base commit `a775806` plus a seven-file uncommitted correction identified by `saturation_fix_source_manifest.json`; every SHA-256 was verified in MATLAB before execution. The run manifest records the working-tree delta. The corrected archive in MATLAB Drive is `gate2_corrected_20260915_233346.zip`, separate from the failed baseline. The local download and extraction are verified: all 88 archive files match, all seven tested-source hashes match the source snapshot and local code, and the saturation checks were independently recalculated from the MATLAB CSV samples. Reports and generated evidence remain excluded from GitHub. The evidence cleanup is complete: 46 readable French/English figures and two 17-page LaTeX reports are stored locally. Final report plots use Matplotlib solely to display verified MATLAB CSV samples; no simulation was rerun. Both the raw archive and the separate native MATLAB re-export are retained. See the [evidence-handling guide](gate2_evidence.en.md). Supervisor acceptance, reserve sensitivity and Gate 3 remain open.
...
The [Gate 3 campaign](gate3.en.md), local draft safety analysis and bilingual progress-report material are now prepared. Reports remain in the ignored `reports/` directory. Review the completed Gate 2 evidence with the supervisor, then execute Gate 3 in MATLAB; review thresholds and assumptions explicitly. The [gate register](project_status.en.md) lists unresolved envelope, authority and fault-coverage items. Dual faults and standardized maneuvers remain extensions.
