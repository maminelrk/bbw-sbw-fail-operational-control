...
Les sources testées sont le commit `a775806` complété par sept fichiers corrigés non commités, identifiés dans `saturation_fix_source_manifest.json` ; chaque SHA-256 a été vérifié dans MATLAB avant exécution. Le manifeste du calcul enregistre ces modifications. L'archive corrigée dans MATLAB Drive est `gate2_corrected_20260915_233346.zip`, séparée de l'échec initial. Le téléchargement et l'extraction locaux sont vérifiés : les 88 fichiers correspondent à l'archive, les sept empreintes sources correspondent à l'instantané testé et au code local, et les contrôles de saturation ont été recalculés depuis les CSV MATLAB. Rapports et preuves générées restent exclus de GitHub. Le dossier de preuves est désormais mis en forme : 46 figures lisibles FR/EN et deux rapports LaTeX de 17 pages sont conservés localement. Les figures finales utilisent Matplotlib uniquement pour afficher les CSV MATLAB vérifiés, sans nouvelle simulation. L'archive brute et le réexport MATLAB natif distinct sont conservés. Voir le [guide des preuves](gate2_evidence.fr.md). Acceptation de l'encadrant, sensibilité de la réserve et Gate 3 restent ouverts.
